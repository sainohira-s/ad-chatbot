            ...
            if(resultSchedule.rowCount > 0){
                for(var i=0; i<resultSchedule.rowCount; i++){
                    ...
                    if (JSON.parse(cron)) {
                        ...
                        console.log('スケジュール')
                        let scheduler = new schedule.scheduleJob("Good morning! Today's Question!", '0 0 11 ' + dateJson.date + ' *', function(){
                        ...
                        let schedulerAfter = new schedule.scheduleJob("Today's Question Result!", '0 5 11 ' + dateJson.date + ' *', function(){
                            let clientSche = new pg.Client(conString);
                            clientSche.connect((err) => {
                                if(err) {
                                    console.log('[scheduleMessage]DB connected failed.', err);
